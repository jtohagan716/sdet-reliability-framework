...
test('protected patient summary rejects missing token', async ({ request }) => {
  ...
  expect(payload.detail).toBe('MISSING_AUTHORIZATION_HEADER');
});


test('protected patient summary rejects invalid token', async ({ request }) => {
  const response = await request.get(
    'http://127.0.0.1:8000/secure/patient-summary',
    {
      headers: {
  ...
  expect(payload.detail).toBe('INVALID_TOKEN');
});


test('protected patient summary rejects wrong role', async ({ request }) => {
...
test('protected patient summary allows valid provider token', async ({ request }) => {
  const token = buildTestJwt({
    sub: 'james',
    role: 'provider',
    iss: 'https://company-login.com',
