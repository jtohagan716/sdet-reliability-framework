"""
Docker-backed validation for per-table synchronization results.

This test verifies table-result constraints, uniqueness, referential
integrity, and rollback behavior against PostgreSQL.
"""
...
VALIDATION_SCRIPT = (
    REPO_ROOT
    / "scripts"
    / "validate_clinical_reference_sync_table_result.sql"
)
...
def apply_schema() -> None:
    """Apply the repeatable synchronization schema."""
...
def test_sync_table_result_constraints_and_rollback():
    """Validate per-table result integrity and rollback."""
    ...
    assert result.returncode == 0, (
        "Synchronization table-result validation failed.\n\n"
        f"STDOUT:\n{result.stdout}\n\n"
        f"STDERR:\n{result.stderr}"
    )
    ...
    assert count_pattern.search(combined_output), (
        "Expected exactly one valid table-result record.\n\n"
        f"OUTPUT:\n{combined_output}"
    )
    ...
    assert rollback_check.returncode == 0, (
        "Table-result rollback verification failed.\n\n"
