"""
Validation for incremental synchronization without a checkpoint.

The test proves that the procedure fails safely, preserves the existing
facility cache, prevents inserts and updates, records failure evidence,
and does not invent a checkpoint.
"""
...
VALIDATION_SCRIPT = (
    REPO_ROOT
    / "scripts"
    / "validate_clinical_reference_incremental_missing_checkpoint.sql"
)
...
def test_incremental_sync_without_checkpoint_fails_safely():
    """Validate the missing-checkpoint failure path."""
    ...
    assert result.returncode == 0, (
        "Missing-checkpoint validation failed.\n\n"
        f"STDOUT:\n{result.stdout}\n\n"
        f"STDERR:\n{result.stderr}"
    )
    ...
    assert "Telehealth Visit" in combined_output
        ...
        SELECT
            ...
            (
                ...
                WHERE reference_domain = 'appointment_type'
                  AND last_successful_sync_run_id IN (
                      '44444444-4444-4444-4444-444444444444',
                      '55555555-5555-5555-5555-555555555555'
                  )
    ...
    assert rollback_check.returncode == 0, (
        "Missing-checkpoint rollback verification failed.\n\n"
